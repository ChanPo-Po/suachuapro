...
function initDashboard(){
  ...
  apiCall({action:'getMasters'}).then(r=>{MASTERS=r.data||{}; setupRoleUI(); openTab(defaultTab());}).catch(()=>{setupRoleUI();openTab(defaultTab());});
}
...
function loadAdminOverview(){
  ...
  apiCall({action:'adminOverview',from:monthStart(),to:isoDate(new Date())},{timeoutMs:35000}).then(r=>renderAdminOverview(r.data||{})).catch(e=>el.innerHTML=`<div class="v15-error">${esc(e.message)}</div>`);
}
...
function loadProgress(){const el=document.getElementById('progress');el.innerHTML=progressFilters()+skeleton(4); apiCall({action:'progressList',...PROGRESS_STATE},{timeoutMs:35000}).then(r=>renderProgress(r)).catch(e=>el.innerHTML+=`<div class="v15-error">${esc(e.message)}</div>`)}
